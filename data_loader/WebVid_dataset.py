    ...
    class WebVid(TextVideoDataset):
        ...
        def _load_metadata(self):
            ...
            if self.dataset_name == 'MSRVTT_zyzocranns_official':
                ...
                self.val_anns = [el for el in self.zyz_anns if el['videos_info'][split] == 'test']
                self.test_anns = [el for el in self.zyz_anns if el['videos_info'][split] == 'test']
            elif self.dataset_name == 'WebVid':
                # anns_path = os.path.join(data_dir, 'vitvr/vitvr_822.json')
                anns_path = params['WebVid_train_anns']
                with open(anns_path, 'r') as fr:
                    train_json = json.load(fr)
                    if not os.path.exists(train_json[0]['path']):
                        for i,ann in enumerate(train_json):
                            train_json[i]['path'] = os.path.join(self.data_dir,train_json[i]['path'])
#                             train_json[i]['videos_info']['Kwai_OCR'] = os.path.join(self.data_dir,train_json[i]['videos_info']['Kwai_OCR'])

                    split = 'split'
                
                # load val
                val_path = params['val_anns']
                with open(val_path, 'r') as fr:
                    val_json = json.load(fr)
                    if not os.path.exists(val_json[0]['path']):
                        for i,ann in enumerate(val_json):
                            val_json[i]['path'] = os.path.join(params['val_video_path'],"Video",val_json[i]['path'])
#                             val_json[i]['videos_info']['Kwai_OCR']\
#                             = os.path.join(params['val_video_path'],"Kwai_VideoOCR",val_json[i]['videos_info']['Kwai_OCR'])
                            
                            
                    
                self.train_anns = [el for el in train_json if el['videos_info'][split] == 'train']
                self.val_anns = [el for el in val_json if el['videos_info'][split] == 'test']
                self.test_anns = [el for el in val_json if el['videos_info'][split] == 'test']
            elif "YOUCOOK2SEG" in self.dataset_name:
                anns_path = os.path.join(data_dir, 'youcook2seg/youcook2seg.json')
                with open(anns_path, 'r') as fr:
                    self.zyz_anns = json.load(fr)
...
#                 anns_path = os.path.join(data_dir, 'youcook2seg/youcook2seg.json')
                ...
                train_anns = [el for el in train_json if el['videos_info'][split] == 'train']
                val_anns = [el for el in val_json if el['videos_info'][split] == 'test']
                test_anns = [el for el in val_json if el['videos_info'][split] == 'test']
...
#         def _load_val_ocr_infos(self,el):
#             ocr_path = el['videos_info']['Kwai_OCR']
# #             ocr_path = os.path.join(self.data_dir,ocr_path)
#             if not os.path.exists(ocr_path) or not os.path.isfile(ocr_path):
#                 print(f'{ocr_path} not exists.')
# #                 continue
#             else:
#                 with open(ocr_path, 'r') as fr:
#                     ocr_file = json.load(fr)
#                 el['ocr_info'] = ocr_file        
#             return self.ocr_format(el)    
#             for i,el in enumerate(self.metadata):
#                 ocr_path = el['videos_info']['Kwai_OCR']
# #                 if not os.path.exists(ocr_path):
# #                     ocr_path = ocr_path.replace('/mmu-ocr','/mmu-ocr/pub')
#                 ocr_path = os.path.join(params['val_video_path'],"Kwai_VideoOCR",ocr_path)
...
#                 with open(ocr_path, 'r') as fr:
#                     ocr_file = json.load(fr)
#                 self.metadata[i]['ocr_info'] = ocr_file
#             self.metadata = [self.ocr_format(el) for el in self.metadata]
            
            
        def _load_ocr_infos(self,el):
            ocr_path = el['videos_info']['Kwai_OCR']
#             ocr_path = os.path.join(self.data_dir,ocr_path)
            if not os.path.exists(ocr_path) or not os.path.isfile(ocr_path):
                print(f'{ocr_path} not exists.')
...
#             self.metadata = [self.ocr_format(el) for el in self.metadata]
        ...
        def __getitem__(self, item):
            ...
            sample = self.metadata[item]
#             if self.split == 'train':
            sample = self._load_ocr_infos(sample)
#             else:
#                 sample = self._load_val_ocr_infos(sample)
            video_fp, rel_fp = self._get_video_path(sample)
            caption = self._get_caption(sample)
