...
#                 anns_path = os.path.join(data_dir, 'youcook2seg/youcook2seg.json')
    ...
    class MSRVTT(TextVideoDataset):
        ...
        def _load_metadata(self):
            data_dir = '../'
            if self.dataset_name == 'MSRVTT':
                # anns_path = os.path.join(data_dir, 'vitvr/vitvr_822.json')
                if self.split == 'train':
                    anns_path = params['anns']
                else:
                    anns_path = params['MSRVTT_val_anns']
                with open(anns_path, 'r') as fr:
                    zyz_anns = json.load(fr)
                    if not os.path.exists(zyz_anns[0]['path']):
                    ...
                    split = 'split'
                self.train_anns = [el for el in zyz_anns if el['videos_info'][split] == 'train']
                self.val_anns = [el for el in zyz_anns if el['videos_info'][split] == 'test']
                self.test_anns = [el for el in zyz_anns if el['videos_info'][split] == 'test']
